...
class MrpProduction(models.Model):
    ...
    #comentado por wmoreno porque mrp_production_id no existe
    #analytic_line_ids = fields.One2many(
    #    comodel_name="account.analytic.line", inverse_name="mrp_production_id",
    #    string="Cost Lines")
    analytic_line_ids = fields.Many2one('account.analytic.line',string="Cost Lines")
    ...
    def _prepare_estimated_cost_analytic_line(
        ...
        if product:
            ...
            if not journal:
                if not journal:
                    journal = self.env['account.journal'].search([('code','=','MRP')])
            return {
                'name': name,
                ...
                'date': datetime.now(), #analytic_line_obj._get_default_date(),
    ...
    def _create_material_estimated_cost(self, prod, product_line):
        ...
        if product:
            vals = self._prepare_estimated_cost_analytic_line(journal, name, prod, product, workorder=product_line.work_order,qty=qty)
        # print vals
        # id = self.env['account.analytic.line'].create({'account_id': 23, 'general_account_id': 1121, 
        #         'mrp_production_id': 8, 'estim_std_cost': -0.0, 'estim_avg_cost': -87.0, 
        #         'date': datetime.now(), 
        #         'name': u'PRUEBA-Packing - iPad Retina Display', 'user_id': 1, 
        #         'product_id': 21, 'product_uom_id': 1, 'amount': 0, 'unit_amount': 1.0, 
        #         'workorder': 37})
        # print id
        return True
