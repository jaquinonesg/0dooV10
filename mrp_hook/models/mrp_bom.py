...
class MrpBom(models.Model):
    ...
    def _prepare_consume_line(self, bom_line, quantity, factor=1):
        #comentado por wmoreno porque no existe product_uos
    ...
    def _bom_explode(self, product, factor, properties=None, level=0,
        ...
        for bom_line_id in bom.bom_line_ids:
            ...
            #comentado por wmoreno porque el type no existe
            ...
            #if (bom_line_id.type != "phantom" and
            #        (not bom_id or self.browse(bom_id).type != "phantom")):
            result.append(self._prepare_consume_line(bom_line_id, quantity, factor))
            #
            #elif bom_id:
            if bom_id:
                ...
                #      '"%s" does not have any BoM defined.') %
                    #comentado por wmoreno porque name no existe
                    #(master_bom.name, self._get_bom_product_name(bom_line_id)))
                #    (master_bom.code or None, self._get_bom_product_name(bom_line_id)))
